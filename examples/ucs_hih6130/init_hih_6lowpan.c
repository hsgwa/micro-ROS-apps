...
// When running on nsh prompt
    ...
    sprintf(buffer,"i8sak set panid %02x:%02x", HIH_PAN_ID & 0xff, HIH_PAN_ID  >> 8); //Set network PAN ID
    system(buffer);


    // sprintf(buffer,"i8sak set saddr 42:%02x",CONFIG_UCS_HIH6130_EXAMPLE_ID); // Set the short address of the radio
    sprintf(buffer,"i8sak set saddr 42:%02x",HIH_DEVICE_ID); // Set the short address of the radio
    system(buffer);

    // sprintf(buffer, "i8sak set eaddr 00:fa:de:00:de:ad:be:%02x", CONFIG_UCS_HIH6130_EXAMPLE_ID); // TODO: This won't work on the lastest version of NuttX
    sprintf(buffer, "i8sak set eaddr 00:fa:de:00:de:ad:be:%02x", HIH_DEVICE_ID); // TODO: This won't work on the lastest version of NuttX
    system(buffer);
